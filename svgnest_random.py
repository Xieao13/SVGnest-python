...
class SvgNest:
    ...
    def launch_workers(self, tree, bin_polygon, config, predictions=None):
        ...
        # 设置随机种子（可选）
        random.seed(5555)  # 可以使用任意整数

        # 生成随机的placement_order
        placement_order = list(range(len(tree)))
        random.shuffle(placement_order)

        # 生成随机的rotations
        possible_rotations = [0, 90, 180, 270]
        rotations = [random.choice(possible_rotations) for _ in range(len(tree))]
...
if __name__ == '__main__':
    ...
    output_file = "output/placement_random_0514.jsonl"
