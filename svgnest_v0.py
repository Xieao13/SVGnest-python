...
class SvgNest:
    ...
    def start(self, is_svg=True, predictions=None) -> bool:
        """开始布局计算"""
        if is_svg:
        ...
        def worker(predictions):
            if not self.working:
                ...
                while iteration < max_iterations and not self.working:
                    print(f"\n开始第 {iteration + 1} 次迭代...")
                    self.launch_workers(self.tree, self.bin_polygon, self.config, predictions)
    ...
    def launch_workers(self, tree, bin_polygon, config, predictions=None):
        """启动工作进程"""
        ...
        # 创建放置工作器
        try:
            placelist = [tree[i] for i in placement_order]
...
def load_predictions(prediction_file: str):
    ...
    每次调用返回一行预测结果
    """
    print("load_predictions: 开始加载预测结果...")
    try:
        with open(prediction_file, 'r') as f:
            for line in f:
                try:
                    prediction = json.loads(line)
                    print(f"load_predictions: 成功加载预测结果: {prediction}")
                    yield prediction
                except json.JSONDecodeError as e:
                    print(f"load_predictions: 解析JSON行时出错: {e}")
                    continue
    except Exception as e:
        print(f"load_predictions: 打开文件时出错: {e}")
        return None
...
def main(input_file: str, output_file: str):
    ...
    if '.svg' in input_file:
        ...
        with open(input_file, 'r') as f_in, open(output_file, 'a') as f_out:
            ...
            predictions = load_predictions("src/results/predictions_20250508_150412.jsonl")
            for line in tqdm(lines, total=len(lines), desc="Processing instances"):
                ...
                # 开始布局计算
                print("Starting placement calculation...")
                result = nester.start(is_svg=False, predictions=predictions)
                if not result:
                    print("Error: Failed to start placement calculation")
                    continue
                ...
                if nester.best:
                    ...
                    if i <= 10:
                        # 将完整的结果传递给apply_placment
                        output_svg = nester.apply_placement(nester.best)
                        output_root = f"{output_file.replace('.jsonl', '')}"
                        output_svgfile = os.path.join(output_root, f"placement_{i}.svg")
                        if not os.path.exists(output_root):
                            os.makedirs(output_root)
                        with open(output_svgfile, 'w', encoding='utf-8') as f:
                            f.write(output_svg)
                        print(f"Generated placement results in {output_svgfile}")
...
if __name__ == '__main__':
    # svg_file = "input.svg"
    jsonl_file = "src/results/predictions_20250508_150412.jsonl"
    output_file = "output/placement_mymodel_0515.jsonl"
