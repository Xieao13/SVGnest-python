...
class GeometryUtil:
    ...
    @staticmethod
    def polygon_intersection(combined_nfp, bin_nfp):
        """
        计算combined_nfp图形轮廓的并集的外部轮廓与bin_nfp内部的交集

        参数:
            combined_nfp: 由多个NFP组成的列表，每个NFP是由{x,y}点组成的列表，表示待放置零件不能放置的区域
            bin_nfp: 由{x,y}点组成的列表，表示容器边界（IFP）

        返回:
            由{x,y}点组成的列表，表示可摆放区域
        """
        import pyclipper

        # 定义缩放系数，Clipper库需要整数坐标
        SCALING_FACTOR = 10 ** 6

        # 转换容器边界为Clipper格式
        bin_path = [[p['x'] * SCALING_FACTOR, p['y'] * SCALING_FACTOR] for p in bin_nfp]

        # 初始化Clipper对象，用于计算combined_nfp的并集
        pc = pyclipper.Pyclipper()

        # 添加所有NFP到Clipper对象
        for nfp in combined_nfp:
            # 转换NFP为Clipper格式
            nfp_path = [[p['x'] * SCALING_FACTOR, p['y'] * SCALING_FACTOR] for p in nfp]
            pc.AddPath(nfp_path, pyclipper.PT_SUBJECT, True)

        # 计算combined_nfp的并集
        union_result = pc.Execute(pyclipper.CT_UNION, pyclipper.PFT_NONZERO, pyclipper.PFT_NONZERO)

        # 如果没有结果，表示没有NFP约束，返回整个容器区域
        if not union_result:
            return bin_nfp

        # 初始化新的Clipper对象，用于计算bin_nfp减去union_result的差集
        pc_difference = pyclipper.Pyclipper()

        # 添加bin_nfp作为subject
        pc_difference.AddPath(bin_path, pyclipper.PT_SUBJECT, True)

        # 添加combined_nfp的并集作为clip
        for path in union_result:
            pc_difference.AddPath(path, pyclipper.PT_CLIP, True)

        # 计算差集：bin_nfp - union_result（即容器内部减去NFP区域）
        difference_result = pc_difference.Execute(pyclipper.CT_DIFFERENCE, pyclipper.PFT_NONZERO, pyclipper.PFT_NONZERO)

        # 如果没有差集结果，表示整个容器都被NFP覆盖，没有可放置区域
...
if __name__ == "__main__":
    ...
    bin_nfp = [{'x': 150, 'y': 180},{'x': 0.0, 'y': 180},{'x': 0.0, 'y': 0.0},{'x': 150, 'y': 0.0}]

    combined_nfp=[]
    combined_nfp.append(polygon_a)
    combined_nfp.append(polygon_b)

    final_nfp = GeometryUtil.polygon_intersection(combined_nfp, bin_nfp)
    print(final_nfp)
    final_nfp.append(final_nfp[0])

    import matplotlib.pyplot as plt
    fig, ax = plt.subplots()
    # ax.plot([p['x'] for p in polygon_a], [p['y'] for p in polygon_a], 'r')
    # ax.plot([p['x'] for p in polygon_b], [p['y'] for p in polygon_b], 'b')
    # ax.plot([p['x'] for p in bin_nfp], [p['y'] for p in bin_nfp], 'g')
    ax.plot([p['x'] for p in final_nfp], [p['y'] for p in final_nfp], 'y')
    plt.show()
